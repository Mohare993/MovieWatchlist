...
public class MovieDBRepository implements MovieRepository {
	...
	public String deleteMovie(Long movieId) throws AccountNotFoundException {
		this.manager.remove(this.manager.find(Movie.class, movieId));
		return "SUCCESS - Movie deleted.";
	}

	@Override
	public String getAllMoviesForList(Long listId) {
		TypedQuery<Movie> query = this.manager
				.createQuery("SELECT m from Movie m WHERE MOVIELIST_LISTID='" + listId + "'", Movie.class);
		return this.json.getJSONForObject(query.getResultList());
	}
